...
- 色：各点が録音の何秒目を表しているか（カラーバーで時間を表示）
- 横・縦軸：UMAP による次元削減後の座標（数値そのものは「意味のある尺度」ではなく視覚的な類似度表示です）

### グラフ2：クラスタリング結果の可視化
- 点：短時間フレーム（時間窓）1 点 = 約 0.2 秒（デフォルト）
- 色：同じクラスタに属する点（似た音）
- 横・縦軸：UMAP による次元削減後の座標

### 使い方のヒント
- プロット上で近い点は音響的に似ています
- 時間グラフで、特定の時間帯の音がどこに配置されているかを確認できます
- クラスタグラフで、似た音のパターンを確認できます
- 代表的なクラスタの音を聞いて、人間が意味づけ（このクラスタは「さえずり」か「呼び声」か等）をすると良いです
...
* **各点が録音の何秒目を表しているかを時間カラーバーで表示**
* 時間経過とクラスタリング結果の2つのグラフで多角的に分析可能
...
似た鳴き声は近くに配置されます。時間グラフで特定の時間帯の音の配置を確認し、クラスタグラフで音の種類を確認することで、より詳細な分析が可能になります。
