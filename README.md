...
このプロジェクトは、野鳥の鳴き声を機械学習とUMAPを用いてクラスタリング・可視化する**実験的なプロトタイプ**です。

## 概要

WAVファイルから野鳥の鳴き声を自動抽出し、MFCCなどの音響特徴量を用いてクラスタリングを行い、UMAPで2次元空間に可視化します。
...
### 今後の改善予定
- [ ] パラメータをコマンドラインオプションまたは設定ファイルで指定可能に
- [ ] 複数音声ファイルの一括処理機能
- [ ] より洗練されたクラスタリング手法の検討（GMM、階層的クラスタリングなど）
- [ ] ユーザーによるマニュアルラベリング機能
- [ ] クラスタの品質評価メトリクスの追加
- [ ] 処理のログ記録と実行結果の保存機能
- [ ] GUI化による利用性向上
