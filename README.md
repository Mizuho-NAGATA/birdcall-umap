...
<p align="center">
  <img src="https://github.com/user-attachments/assets/5c9b5964-8775-444e-88cf-07e2e3d56f79" alt="UMAP 3kHz以上" width="600" />
</p>

<p align="center">
  <img src="https://github.com/user-attachments/assets/d7b80941-3b8f-4050-9bc9-0333ccf18a32" alt="nakigoe py_"  width="600"/>
</p>

これは、鳥の鳴き声の可視化を試みる**実験作品**です。  
...
```bash
pip install librosa matplotlib numpy sounddevice soundfile scikit-learn umap-learn scipy
```
あるいは  
```bash
python -m pip install librosa matplotlib numpy sounddevice soundfile scikit-learn umap-learn scipy
```
...
### パラメータの調整が必要
- **ハイパスフィルタの周波数**: 初期値は 3000Hz。エアコンや扇風機の音、人の声を除外することを期待。
- **音声区間検出（top_db値）**: 初期値は 45。
- **フレーム長・ホップ長**: 初期値は 0.25 秒。
- **K-Meansのクラスタ数（k=4）**: 最適値の検証が必要
- **MFCC係数数（n_mfcc=20）**: 最適値の検証が必要

### 既知の問題
- 得られたクラスタ（グループ）が意味のある集まりなのかは、データごとに検証が必要。
